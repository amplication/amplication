...
exports.GitHubActionProvider = GitHubActionProvider;
...
class App {
    constructor(actionProvider, storageProvider) {
        ...
        this.main = () => {
            try {
                ...
                const filesToFind = this.actionProvider.getFilesToFined();
                const foldersToSearch = this.actionProvider.getFoldersToSearch();
                const ignoreFolders = this.actionProvider.getIgnoreFolders();
                const recursive = this.actionProvider.getRecursive();
                console.log(`STARTING FOLDERS SEARCH...`);
                const results = this.findFoldersContainingDockerfile(workingDirectory, filesToFind, foldersToSearch, ignoreFolders, recursive);
                console.log(`The action results payload`, results);
                this.actionProvider.setOutput(results);
            }
            catch (err) {
                this.actionProvider.setFailed(err);
            }
        };
    }
    splitFoldersByFile(workingDirectory, directories, filesToFind) {
        const t = underscore_1.default.groupBy(directories, (element) => {
            return this.storageProvider.directoryContains(`${workingDirectory}/${element}`, filesToFind)
    ...
    findFoldersContainingDockerfile(workingDirectory, filesToFind, foldersToSearch, ignoreFolders, recursive) {
        foldersToSearch = foldersToSearch.filter((directory) => {
            return !ignoreFolders.some((ignoredDirectory) => directory.endsWith(ignoredDirectory));
        });
        const { foldersContainingFiles, foldersNotContainsFiles } = this.splitFoldersByFile(workingDirectory, foldersToSearch, filesToFind);
        if (recursive) {
            if (foldersToSearch.length === 0) {
                return [];
            }
            return foldersContainingFiles.concat(foldersNotContainsFiles
                .map((directory) => {
                ...
                return this.findFoldersContainingDockerfile(workingDirectory, filesToFind, subDirectories, ignoreFolders, recursive);
            }));
        }
        else {
...
var underscoreNodeF = __nccwpck_require__(657);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const github_action_provider_service_1 = __nccwpck_require__(664);
const app_1 = __nccwpck_require__(636);
const storage_provider_service_1 = __nccwpck_require__(920);
const actionProvider = new github_action_provider_service_1.GitHubActionProvider();
const storageProvider = new storage_provider_service_1.FsStorageProvider();
const app = new app_1.App(actionProvider, storageProvider);
app.main();
