...
1. Install dependencies of the monorepo (execute in root directory):
...
2. Update code generated by Prisma

```
npm run prisma:generate
```

3. Build dependencies of the server:

```
npm run build -- --scope @amplication/server --include-dependencies
```

4. Update other generated code
...
7. Get external services up (execute in server directory "packages/amplication-server")

```
npm run docker
```

8. Update application database (execute in server directory "packages/amplication-server")

```
npm run start:db
```

9. Start the development server and watch for changes (execute in server directory "packages/amplication-server")
...
1. Get external services up, if they're not already running

```
npm run docker
```

2. Start the development server and watch for changes, if it's not already running

```
npm run start:watch
```

3. Format files (editors like VSCode can do it for you automatically)

```
npm run format
```

4. Lint files (editors like VSCode come with integration to display those continuously)

```
npm run lint
```

5. Run tests and watch for changes

```
npm run test:watch
```

6. When needed, update Prisma Schema
