...
import { removeESLintComments } from "@amplication/code-gen-utils";
...
export async function createEntityGrpcControllerSpec(
  resource: string,
  entity: Entity,
  entityType: string,
  ...
  const template = undefined; // await readFile(testTemplatePath);
  ...
  return pluginWrapper(
    createEntityGrpcControllerSpecInternal,
    EventNames.CreateEntityGrpcControllerSpec,
    {
      entity,
      entityType,
      template,
...
export async function createEntityGrpcControllerSpecInternal({
  template,
  templateMapping,
  entityServiceModulePath,
  entityControllerModulePath,
  ...
  controllerId,
  serviceId,
}: CreateEntityGrpcControllerSpecParams): Promise<ModuleMap> {
  if (!template) return;
