...
export class GithubService implements IGitClient {
  ...
  async createPullRequest(
    ...
    const doNotOverride = [
      new RegExp(`^server\/src\/[^\/]+\/.+\.controller.ts$`),
      new RegExp(`^server\/src\/[^\/]+\/.+\.resolver.ts$`),
      new RegExp(`^server\/src\/[^\/]+\/.+\.service.ts$`),
      new RegExp(`^server\/src\/[^\/]+\/.+\.module.ts$`),
      new RegExp(`^server\/scripts\/customSeed.ts$`)
    ];
