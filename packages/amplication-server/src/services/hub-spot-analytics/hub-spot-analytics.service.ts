...
const hubspotClient = new Client({ defaultHeaders: { authorization: `Bearer ${process.env.HUBSPOT_TOKEN}` } });
...
export class HubSpotAnalyticsService {
  async addWorkspace(email: string, workspace: string): Promise<SimplePublicObject> {
    ...
    if (!contact) return;
    const workspaceSet = new Set(contact.properties?.workspace_id ? contact.properties.workspace_id.split(workspaceSeparator) : []);
  ...
  private getContact(contactId: string, properties: string[] = []): Promise<SimplePublicObjectWithAssociations> {
    return hubspotClient.crm.contacts.basicApi.getById(contactId, properties, undefined, undefined, undefined, 'email').catch(() => null);
  }
