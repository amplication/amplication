...
export class GqlAuthGuard extends AuthGuard('jwt') {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    if (roles) {
      ...
      if (!this.matchRoles(roles, currentUser.userRoles.map(r=>r.role))){
        return false;
      };
    }

   const resourceBasedAuth  =  this.reflector.get<ResourceBasedAuthParams>('resourceBasedAuth', context.getHandler());
    ...
    return applyFromContext || this.validateResourceBasedAuthorization(type,val,currentUser);
    
  }

  //validates if the user have permissions to access the requested object
  async validateResourceBasedAuthorization(parameterType : string, parameterValue : string, user: User) : Promise<boolean>{

    switch(parameterType) { 
      case "organizationId": { 
        return this.permissionsService.UserCanAccessOrganization(user, parameterValue);
        break; 
      } 
      case "projectId": { 
        return this.permissionsService.UserCanAccessProject(user, parameterValue);
        break; 
      } 
      default: { 
         return false;
